...
func TestGet(t *testing.T) {
	...
	tt := []struct {
		...
		{give: `[3]`, when: `(flatten)`, then: `3`},
		// (first) (last)
		{give: `{"a":[]}`, when: `(last  a)`, then: ``},
		{give: `{"a":[]}`, when: `(first a)`, then: ``},
		{give: `{"a":[3,4,5]}`, when: `(last  a)`, then: `5`},
		{give: `{"a":[3,4,5]}`, when: `(first a)`, then: `3`},
		{give: `[]`, when: `(last)`, then: ``},
		{give: `[]`, when: `(first)`, then: ``},
		{give: `{"a":[{"b":{"c":3}},{"b":{}}]}`, when: `(last  a b c)`, then: `3`},
		{give: `{"a":[{"b":{"c":3}},{"b":{}}]}`, when: `(first a b c)`, then: `3`},
		{give: `{"a":[{"b":3},{"b":4}]}`, when: `(last  a b)`, then: `4`},
		{give: `{"a":[{"b":3},{"b":4}]}`, when: `(first a b)`, then: `3`},
