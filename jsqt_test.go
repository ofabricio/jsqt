...
func TestGet(t *testing.T) {
	...
	tt := []struct {
		...
		// (collect)
		{
			...
			when: `(get a b c (collect (obj x d e (get e f (collect g h i j (collect k l) (flatten)))))`,
			then: `[{"x":"one","e":[3,4]},{"x":"two","e":[5]}]`,
		},
		{give: `[3,4,5]`, when: `(collect)`, then: `[3,4,5]`},
		{give: `[{"a":3}]`, when: `(collect a)`, then: `[3]`},
		{give: ``, when: `(collect)`, then: `[]`},
		{give: `3`, when: `(collect)`, then: `[3]`},
		{give: `[]`, when: `(collect a)`, then: `[]`},
		{give: `[]`, when: `(collect)`, then: `[]`},
		{give: `[{"b":{"c":3}},{"b":{}}]`, when: `(collect b c)`, then: `[3]`},
